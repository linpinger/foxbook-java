...
public class FoxBookLib {
    	...
    	public static void updatepage(int pageid, FoxDB oDB) {
		...
		updatepage(fullPageURL, pageid, oDB) ;
	}
        ...
        try {
            if (sbaseurl.contains("zhuishushenqi.com")) {     // 对于这种非正常合成URL需处理一下 http://xxx.com/fskd/http://wwe.comvs.fs
                if (suburl.contains("zhuishushenqi.com")) {
                    ...
                    return "http://chapter.zhuishushenqi.com/chapter/" + URLEncoder.encode(suburl, "UTF-8");
                }
            }
